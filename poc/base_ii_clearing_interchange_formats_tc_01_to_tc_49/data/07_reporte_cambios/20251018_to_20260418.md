...
- Cambios de contenido: 193 totales — business_rule_change=91, editorial_reword=97, extraction_noise=5
...
## Cambios de negocio a revisar (91)
...
- **`133-136`** — Merchant Category Code
  - Ficha (description)
    ...
    - Razón (IA): La edición nueva añade una restricción adicional que el campo debe ser el mismo que en la transacción original en disputas financieras, lo que modifica el significado del campo.
  - Ficha (note)
    ...
    - Ahora: ''
    - Razón (IA): El texto antiguo especificaba que el Merchant Category Code debe coincidir con la transacción original, mientras que el nuevo texto está vacío, lo que implica una redefinición del campo.
- **`150`** — Settlement Flag
  - Ficha (description)
...
- **`152-157`** — Authorization Code
  - Ficha (description)
    ...
    - Razón (IA): La edición nueva incluye reglas adicionales de validación específicas para diferentes regiones y tipos de transacciones, como el uso de códigos de autorización específicos para transacciones de avión, EPS y otros servicios, lo que modifica el significado de los valores permitidos.
  - Ficha (note)
    ...
    - Ahora: ''
    - Razón (IA): El texto antiguo define valores específicos de Authorization Code que son inválidos en diferentes contextos, mientras que el texto nuevo está vacío, lo que indica que se eliminó la regla de validación para estos valores, cambiando el significado de negocio.
- **`168`** — Reimbursement Attribute
  - Ficha (description)
    ...
    - Razón (IA): La nueva edición añade una referencia a la documentación BASE II Clearing Data Codes para definiciones de Reimbursement Attribute, lo que modifica el contexto de validación del campo.
  - Ficha (note) ⚠️ *(corregido por contenido desaparecido)*
...
- **`92-116`** — Merchant Name
  - Ficha (description)
    ...
    - Razón (IA): La nueva edición añade restricciones específicas para programas de tarifas aéreas y transacciones VisaPhone, modificando las reglas de negocio para el campo Merchant Name.
  - Ficha (note)
    - Antes: 'Some airline fee programs may require the original ticket number (ticket identifier) or the ancillary service description in positions 13 through 25 of the merchant name. VisaPhone transactions (MCC 4815) must be in the following format: Dispute Response Financials The field must be the same as in the original transaction unless a correction is required to resolve a dispute financial. Dispute Financials, Reversals The entry must be the same as in the original transaction.'
    - Ahora: ''
    - Razón (IA): El texto antiguo especifica que VisaPhone transactions (MCC 4815) deben seguir un formato particular en el campo Merchant Name, mientras que el texto nuevo está vacío, lo que implica una redefinición del campo o una condición de negocio que cambió.
...
- **`20-31`** — Amount Base of Add Value Tax to Return
  ...
  - Grilla (Field Length)
    - Antes: '8'
    - Ahora: '12'
    - Razón (IA): El campo de longitud cambia de 1 a 12 bytes, lo que implica un cambio en la cantidad de datos que se espera para el valor de la tarjeta de crédito en Colombia.
  - Ficha (positions)
    - Antes: '88–95'
    - Ahora: '20–31'
    - Razón (IA): El rango de posiciones del campo cambia de 88-95 a 20-31, lo que redefine el layout del mensaje y afecta la interpretación del campo en el formato de intercambio.
  - Ficha (length)
    - Antes: '8'
    - Ahora: '12'
    - Razón (IA): El campo 'length' cambia de 8 a 12 bytes, lo que implica un cambio en la longitud válida del campo de valor de la tarjeta, afectando la estructura de datos y el significado del mensaje.
...
- **`32-43`** — Consumption Tax Amount
  - Grilla (Position)
    - Antes: '96–104'
    - Ahora: '32–43'
    - Razón (IA): El rango de posición del campo de Consumo de Impuesto cambió de 96-104 a 32-43, lo que implica una redefinición del layout del mensaje.
  - Grilla (Field Length)
    - Antes: '9'
    - Ahora: '12'
    - Razón (IA): El campo de longitud del monto de impuesto de consumo cambió de 1 a 2 bytes, lo que implica un cambio en la validación de datos y el rango de valores permitidos.
  - Ficha (positions)
    - Antes: '96–104'
    - Ahora: '32–43'
  ...
  - Ficha (length)
    - Antes: '9'
    - Ahora: '12'
    - Razón (IA): El campo 'length' cambia de 1 a 2 bytes, lo que implica un cambio en la longitud del campo de consumo de impuestos, afectando la validación y estructura de los mensajes.
  - Ficha (description) ⚠️ *(corregido por contenido desaparecido)*
    ...
    - Razón (IA): El texto se reescribió para eliminar los puntos y usar viñetas, manteniendo el mismo significado.
- **`44-55`** — Add Value Tax Amount
  - Grilla (Position)
    - Antes: '105–113'
    - Ahora: '44–55'
    - Razón (IA): El rango de posición del campo cambia de 105-113 a 44-55, lo que redefine su ubicación en el mensaje y afecta el formato de intercambio.
  - Grilla (Field Length)
    - Antes: '9'
    - Ahora: '12'
    - Razón (IA): El campo de longitud cambia de 1 a 12 bytes, lo que implica un cambio en la cantidad de datos que se procesa, afectando la validación y el formato del mensaje.
  - Ficha (positions)
    - Antes: '105–113'
    - Ahora: '44–55'
    - Razón (IA): El rango de posiciones del campo cambia de 105-113 a 44-55, lo que redefine el layout del mensaje y afecta la estructura de datos.
  - Ficha (length)
    - Antes: '9'
    - Ahora: '12'
    - Razón (IA): El campo 'length' cambia de 9 a 12 bytes, lo que implica un cambio en la longitud válida del campo de valor fiscal, afectando la estructura de datos y el significado del mensaje.
...
- **`139`** — Net/Gross Flag
  - Ficha (description)
    ...
    - Razón (IA): La nueva edición especifica que deben establecerse valores 1, 2, 8 o 9, y si no se usa el campo debe ser 1 (sin espacio o 0), lo que modifica la regla de negocio respecto a los valores permitidos.
  - Ficha (note)
    - Antes: '1, 2, 8 or 9 must be set. If not used set 1 (no space or 0).'
    - Ahora: ''
    - Razón (IA): La edición vieja especificaba que debe ser 1, 2, 8 o 9, mientras que la nueva está vacía, lo que implica una redefinición del campo sin condiciones válidas.
- **`160-167`** — Acquirer's Invoice Amount
  - Ficha (description)
    ...
    - Razón (IA): El texto nuevo especifica que el campo debe ser cero rellenado para entradas entrantes y contiene el valor calculado para salidas, mientras que la versión anterior solo requería ser numérico.
- **`26-33`** — Net Sales Amount
  - Ficha (description)
    ...
    - Razón (IA): La nueva edición añade una condición específica que el valor debe coincidir con el TCR 0 Source Amount si el Tax and Mailing Charge es cero, lo que modifica la lógica de negocio.
  - Ficha (note)
...
- **`34-41`** — Tax and Mailing Charge
  - Ficha (description)
    ...
    - Razón (IA): La nueva edición añade una regla de negocio que relaciona TCR 2 Net Sales Amount con Tax and Mailing Charge para calcular TCR 0 Source Amount, modificando la lógica de validación.
  - Ficha (note)
    - Antes: '* TCR 2 Net Sales Amount (position 26–33) + Tax and Mailing Charge (34–41) = TCR 0 Source Amount (= Destination Amount).'
    - Ahora: ''
    - Razón (IA): El texto antiguo define una relación matemática entre campos que ahora está vacío, lo que implica que la regla de negocio cambió significativamente.
...
- **`149`** — Spend Qualified Indicator
  - Ficha (description) ⚠️ *(corregido por contenido desaparecido)*
    ...
    - Razón (IA): El texto solo cambió los puntos de lista de los valores con asteriscos en lugar de puntos y corrigió un espacio extra en 'reclassifi- cations' para mantener la coherencia en el formato de texto.
...
- **`77-146`** — Message Text
  - Ficha (name) ⚠️ *(corregido por contenido desaparecido)*
    ...
    - Ahora: 'Message Text'
    - Razón (IA): El texto describe el mismo concepto pero con una redacción más simple y directa, eliminando detalles específicos como el código de razón.
  - Ficha (description)
    ...
    - Razón (IA): El texto define nuevas reglas de uso para el campo en transacciones específicas (Reason Code 0300, 5220, 5230) y establece requisitos adicionales como evitar espacios en blanco y contenido de CEDP, lo que cambia el significado operativo del campo.
...
- **`4`** — Transaction Component Sequence Number
  - Ficha (name)
    ...
    - Razón (IA): El campo se redefinio de 'Transaction Code Qualifier Number' a 'Transaction Component Sequence Number', cambiando su significado técnico y de negocio.
...
- **`21-30`** — Merchant Verification Value
  - Ficha (description)
    ...
    - Razón (IA): La nueva edición añade reglas de valores (A-F, 0-9) y un valor predeterminado (espacios), modificando las reglas de negocio del campo.
  - Ficha (values)
    - Antes: 'A-F, 0-9 l Default value: spaces l'
    - Ahora: ''
    - Razón (IA): El valor predeterminado cambió de espacios a vacío, lo que implica una redefinición del comportamiento del campo.
...
- **`100`** — Authorization Characteristics Indicator
  - Ficha (description)
    ...
    - Razón (IA): La nueva descripción incluye información adicional sobre los códigos válidos y su población basada en la autorización crediticia, lo que modifica el significado del campo.
  - Ficha (note) ⚠️ *(corregido por contenido desaparecido)*
...
- **`101`** — POS Terminal Capability
  - Ficha (description)
    ...
    - Razón (IA): La nueva edición añade información sobre los códigos válidos y la lógica de llenado, modificando el significado del campo.
  - Ficha (note) ⚠️ *(corregido por contenido desaparecido)*
    - Antes: 'Please see BASE II Clearing Data Codes for a list of valid codes. Populated based on Credit Authorization.'
    - Ahora: ''
    - Razón (IA): El texto en la edición nueva está vacío, lo que sugiere un artefacto de extracción de PDF en lugar de un cambio real de contenido.
...
- **`136`** — POS Environment
  - Ficha (description)
    ...
    - Razón (IA): La nueva edición añade información sobre códigos válidos y especifica que se rellena con espacios para créditos, lo que modifica el comportamiento del campo en términos de validación y uso.
  - Ficha (note) ⚠️ *(corregido por contenido desaparecido)*
    - Antes: 'Please see BASE II Clearing Data Codes for a list of valid codes. Space-filled for credits.'
    - Ahora: ''
    - Razón (IA): El texto nuevo está vacío, lo que sugiere un artefacto de extracción de PDF en lugar de un cambio real de contenido.
- **`15-16`** — POS Entry Mode
  - Ficha (description)
    ...
    - Razón (IA): La nueva edición añade información sobre los valores válidos y el comportamiento de llenado del campo, modificando su significado en términos de negocio.
  - Ficha (note) ⚠️ *(corregido por contenido desaparecido)*
...
- **`42`** — AVS Response Code
  - Ficha (description)
    ...
    - Razón (IA): La nueva edición añade información sobre los códigos válidos y el relleno espaciado para créditos, lo que modifica el significado del campo en términos de validación y uso.
  - Ficha (note) ⚠️ *(corregido por contenido desaparecido)*
    - Antes: 'Please see BASE II Clearing Data Codes for a list of valid codes. Space-filled for credits.'
    - Ahora: ''
    - Razón (IA): El texto nuevo está vacío, lo que sugiere un artefacto de extracción de PDF en lugar de un cambio real de contenido.
...
- **`9-14`** — Authorization Code
  - Ficha (description)
    ...
    - Razón (IA): La nueva edición añade requisitos específicos sobre la gestión del código de autorización según los requisitos del esquema de tarjeta, lo que modifica la regla de negocio.
  - Ficha (note) ⚠️ *(corregido por contenido desaparecido)*
    - Antes: 'Please see BASE II Clearing Data Codes for a list of valid codes. Populated based on Credit Authorization.'
    - Ahora: ''
    - Razón (IA): El texto en la edición nueva está vacío, lo que sugiere un artefacto de extracción de PDF en lugar de un cambio real de contenido.
- **`91-92`** — Product ID
  - Ficha (description)
    ...
    - Razón (IA): La nueva edición especifica que el campo se rellena con espacios para créditos, lo que modifica el comportamiento del campo en términos de formato de datos.
- **`93-98`** — Program ID
  - Ficha (description)
...
- **`17-18`** — Payment Mode
  - Ficha (description)
    ...
    - Razón (IA): La nueva edición añade una referencia a los códigos válidos en BASE II, lo que modifica la información sobre los valores permitidos para el campo.
  - Ficha (note) ⚠️ *(corregido por contenido desaparecido)*
...
- **`59-78`** — Tax Document Type
  - Ficha (description)
    ...
    - Razón (IA): El cambio incluye nuevas identificaciones de país (Colombia y Perú) y diferentes formatos de documentos tributarios, lo que altera los valores válidos para el campo Tax Document Type.
...
Identifier
  - Grilla (Position)
    - Antes: '74-109'
    - Ahora: '73-108'
    - Razón (IA): El rango de posiciones cambió de 74-109 a 73-108, lo que afecta el tamaño del campo y su definición en el formato.
  - Ficha (positions)
    - Antes: '74-109'
    - Ahora: '73-108'
    - Razón (IA): El rango de posiciones del campo cambió de 74-109 a 73-108, lo que afecta el tamaño y el rango válido del campo en el formato de mensaje.
...
- **`153-164`** — Retrieval Reference Number
  - Ficha (description) ⚠️ *(corregido por contenido desaparecido)*
    ...
    - Razón (IA): El texto describe lo mismo pero reemplaza los puntos con guiones circulares para los tipos de transacciones, sin cambiar el significado o valores válidos.
...
## Apéndice: cambios de solo redacción / ruido de extracción (102)
...
- **`28-33`** — Acquirer Reference Number— Acquiring Identifier
  - Ficha (name) — editorial_reword (similitud 0.60)
...
- **`74-79`** — Processing Code
  - Ficha (description) — editorial_reword (similitud 0.51)
- **`80-83`** — Network ID
  - Ficha (description) — editorial_reword (similitud 0.73)
- **`84-85`** — Authorization Response Code
  - Ficha (description) — editorial_reword (similitud 0.70)
...
- 38 cambio(s) de negocio fueron re-clasificados automáticamente porque la IA los marcó como redacción/ruido pero se detectó contenido real desaparecido sin reemplazo (ver marca ⚠️ arriba).
